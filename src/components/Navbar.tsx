    ...
    <motion.nav className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300 w-full", isScrolled ? "bg-white shadow-sm" : "bg-black")} initial={{
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <Link to="/" className="flex items-center">
              <span className={cn("text-5xl font-bold", isScrolled ? "text-gray-900" : "text-white")}>MAQ</span>
              <span className={cn("ml-2 text-2xl font-bold", isScrolled ? "text-gray-900" : "text-white")}>Technologies</span>
            </Link>
          </div>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent>
                    <ul className="grid gap-3 p-4 w-[400px]">
                      <li>
                        <Link to="/projects/garagat" className="block p-3 space-y-1 rounded-md hover:bg-gray-100">
                          <div className="font-medium">Garagat.ai</div>
                          <p className="text-sm text-gray-500">Smart automotive service marketplace</p>
                        </Link>
                      </li>
                      <li>
                        <Link to="/projects/jordan-stays" className="block p-3 space-y-1 rounded-md hover:bg-gray-100">
                          <div className="font-medium">JordanStay Homes</div>
                          <p className="text-sm text-gray-500">Tech-enabled short-term rental platform</p>
                        </Link>
                      </li>
      ...
      <div className={cn("md:hidden transition-all duration-300 overflow-hidden w-full", isMenuOpen ? "max-h-80 opacity-100" : "max-h-0 opacity-0")}>
        <div className={cn("px-3 pt-2 pb-3 space-y-1 shadow-sm overflow-y-auto max-h-80", isScrolled ? "bg-white" : "bg-black")}>
          ...
          <Link to="/projects/garagat" className={cn("block px-3 py-1.5 rounded-md text-sm", isScrolled ? "text-gray-700 hover:bg-gray-50" : "text-gray-200 hover:bg-gray-900")} onClick={() => {
            setIsMenuOpen(false);
            window.scrollTo(0, 0);
          }}>
            Customer Cases
